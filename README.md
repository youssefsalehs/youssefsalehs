<img src="https://capsule-render.vercel.app/api?type=waving&height=200&color=gradient&customColorList=12,14,17,19&text=Youssef%20Ashraf&fontSize=48&fontColor=ffffff&animation=twinkling&fontAlignY=35&desc=Junior%20MERN%20Stack%20Developer%20%7C%20React%20Specialist&descSize=18&descAlignY=55" style="width:100%; height:auto;"/>

<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=26&duration=3500&pause=1200&color=0A4D68&center=true&vCenter=true&width=650&lines=MERN+Stack+Developer;React+%7C+Redux+%7C+TanStack+Query;Building+Scalable+Web+Applications;Clean+Code+%7C+Modern+UI" />
</div>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=youssefsalehs&label=Profile%20Views&color=0A4D68&style=flat-square" />
</div>

---

## 👋 About Me

I’m **Youssef Ashraf**, a **Junior MERN Stack Developer** with hands-on experience building **responsive, user-focused web applications** using modern frontend and backend technologies.

I specialize in **React** and enjoy creating **clean, scalable UIs**, integrating APIs, and building full-stack solutions with strong attention to **performance, UX, and best practices**.

- 🎓 **Computer Engineering**, Mansoura University — *Excellent* (Graduation: 2026)
- ⚛️ Strong focus on **React ecosystem**
- 🌐 MERN Stack (MongoDB, Express, React, Node.js)
- 🤝 Team player with Agile mindset
- 🌍 Based in **Cairo, Egypt** | Open to **internships, junior roles & freelance**

---

## 🚀 Featured Projects

### 🏥 Medicore – Smart Pharmacy App
A full-stack pharmacy platform with secure authentication and scalable backend.

- Browse & order medicines
- Authentication & protected routes
- Responsive modern UI
- MongoDB-powered backend

**Tech Stack:** React, Node.js, Express, MongoDB, Mongoose, Tailwind CSS

---

### 🎬 Verigo – Movie App
A modern movie browsing application with smooth navigation.

- Search & explore movies
- Save favorites using `localStorage`
- React Router navigation
- Global state with Context API

**Tech Stack:** React, Vite, Tailwind CSS, Context API

---

### 📰 NewsMag
A responsive news web application with REST API integration.

- Browse news by category & country
- Light / Dark mode
- Pagination
- Mobile-first design

**Tech Stack:** React, Bootstrap, REST APIs

---

### 🛍 Shopie – Clothing E-Commerce Website
A clean, modern e-commerce UI.

- Reusable navbar & footer
- Custom color palette
- Fully responsive layout
- Home, Shop, About & Contact pages

**Tech Stack:** HTML, CSS, Bootstrap, JavaScript

---

### 🧬 Pokemondex
A React app for exploring Pokémon.

- Search & filter Pokémon
- Detailed Pokémon info
- Interactive UI

**Tech Stack:** React, APIs, CSS

---

## 🛠 Tech Stack

### ✅ Frontend
<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg" width="48"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" width="48"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" width="48"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" width="48"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" width="48"/>
  <img src="https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg" width="48"/>
  <img src="https://www.vectorlogo.zone/logos/getbootstrap/getbootstrap-icon.svg" width="48"/>
</p>

### ⚙️ Backend & Databases
<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original.svg" width="48"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/express/express-original.svg" width="48"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original.svg" width="48"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" width="48"/>
</p>

### 🧰 Tools & Workflow
<p align="center">
  <img src="https://www.vectorlogo.zone/logos/git-scm/git-scm-icon.svg" width="48"/>
  <img src="https://www.vectorlogo.zone/logos/github/github-icon.svg" width="48"/>
  <img src="https://www.vectorlogo.zone/logos/visualstudio_code/visualstudio_code-icon.svg" width="48"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postman/postman-original.svg" width="48"/>

</p>

---

## 🤝 Let’s Connect

<p align="center">
  <a href="https://www.linkedin.com/in/youssef-ashraf-4b4a03267/" target="_blank">
    <img src="https://raw.githubusercontent.com/gauravghongde/social-icons/master/SVG/Color/LinkedIN.svg" width="40"/>
  </a>
  <a href="mailto:youssef.ashraf.saleh@gmail.com" target="_blank">
    <img src="https://raw.githubusercontent.com/gauravghongde/social-icons/master/SVG/Color/Gmail.svg" width="40"/>
  </a>
</p>

---

💡 **Open to Junior Frontend / MERN roles, internships, and freelance work.**  
If you’re looking for someone who cares about **clean code, scalability, and UX** — let’s build something great 🚀

